...
CSV_FILE_PATH = 'out.csv' # Main output CSV
DATA_ENTRY_FILE_PATH = 'data_entries.csv' # Input configuration CSV
...
def calculate_classical_benchmarks_qubo(distance_matrix, all_nodes, A=QUBO_PENALTY_A, B=QUBO_WEIGHT_B):
    ...
    costs = []
    # num_cities here is len(all_nodes)
...
# --- Core QAOA execution function ---
def run_single_qaoa_experiment(method_param, cluster_type_param, target_id_param, qaoa_p_param, shots_param):
    """
    Executes a single QAOA experiment based on the provided parameters.
    Appends results to CSV_FILE_PATH.
    """
    print(f"\n--- Running Experiment: Method={method_param}, Cluster={cluster_type_param}, ID={target_id_param}, P={qaoa_p_param}, Shots={shots_param} ---")
    current_timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

    # STEP 0: Load VRP Data
    distance_matrix, all_nodes, depot_node, customer_nodes = load_vrp_data(JSON_FILE_PATH, cluster_type_param, target_id_param)
    if distance_matrix is None:
        print("Failed to load VRP data. Skipping this experiment.")
        return False # Indicate failure

    # STEP 1: Qubit Allocation and Objective Function Selection
    num_qubits = 0
    qaoa_circuit_func = None
    objective_func_wrapper = None # This will hold the lambda for objective function

    if method_param == 'node_visit_time':
        total_nodes = len(all_nodes)
        num_customers = len(customer_nodes)
        if num_customers == 0:
            print("No customers to route for node_visit_time method. Skipping this experiment.")
            return False
        ...
        def qaoa_objective_function_nvt_internal(params, sampler_obj, num_qubits_obj, p_obj, dist_matrix_obj, num_customers_obj, bits_per_node_obj, depot_node_obj, customer_nodes_obj, total_nodes_obj, backend_obj):
            gamma = params[:p_obj]
            beta = params[p_obj:]
            # ADDED: Print current gamma and beta parameters for node_visit_time
            print(f"  [QAOA Obj] Current parameters: gamma={np.round(gamma, 3)}, beta={np.round(beta, 3)}")
            qc = qaoa_circuit_func(num_qubits_obj, p_obj, gamma, beta)
            qc_transpiled = transpile(qc, backend=backend_obj)
            job = sampler_obj.run([qc_transpiled], shots=shots_param)
            result = job.result()
            meas_data = result[0].data.c
            counts = format_measurement_counts(meas_data)
            expected_cost = sum(
                (count / shots_param) * total_cost_from_bitstring_nvt(bitstring, dist_matrix_obj, num_customers_obj, bits_per_node_obj, depot_node_obj, customer_nodes_obj, total_nodes_obj)
                for bitstring, count in counts.items()
            )
            print(f"  [QAOA Obj] Expected Cost calculated: {expected_cost:.3f}")
            return expected_cost
        
        objective_func_wrapper = lambda params, sampler_obj, backend_obj: qaoa_objective_function_nvt_internal(params, sampler_obj, num_qubits, qaoa_p_param, distance_matrix, num_customers, bits_per_node, depot_node, customer_nodes, total_nodes, backend_obj)
            
    elif method_param == 'naive_adjacency':
        num_cities_qubo = len(all_nodes)
        if num_cities_qubo == 0:
            print("No cities to route for naive_adjacency method. Skipping this experiment.")
            return False
        ...
        def qaoa_objective_function_qubo_internal(params, sampler_obj, num_cities_obj, p_obj, dist_matrix_obj, all_nodes_obj, backend_obj):
            gamma = params[:p_obj]
            beta = params[p_obj:]
            # ADDED: Print current gamma and beta parameters for naive_adjacency (QUBO)
            print(f"  [QAOA Obj] Current parameters: gamma={np.round(gamma, 3)}, beta={np.round(beta, 3)}")
            qc = qaoa_circuit_func(num_cities_obj, p_obj, gamma, beta)
            qc_transpiled = transpile(qc, backend=backend_obj)
            job = sampler_obj.run([qc_transpiled], shots=shots_param)
            result = job.result()
            meas_data = result[0].data.c
            counts = format_measurement_counts(meas_data)
            expected_cost = sum(
                (count / shots_param) * calculate_cost_qubo(decode_bitstring_qubo(bitstring, num_cities_obj), dist_matrix_obj, all_nodes_obj)
                for bitstring, count in counts.items()
            )
            print(f"  [QAOA Obj] Expected Cost calculated: {expected_cost:.3f}")
            return expected_cost

        objective_func_wrapper = lambda params, sampler_obj, backend_obj: qaoa_objective_function_qubo_internal(params, sampler_obj, num_cities_qubo, qaoa_p_param, distance_matrix, all_nodes, backend_obj)
    else:
        print(f"Invalid METHOD '{method_param}' selected. Skipping this experiment.")
        return False

    if num_qubits == 0:
        print("No qubits allocated. Skipping this experiment.")
        return False
    ...
    print(f"\nSTEP 3: Starting classical optimization for QAOA (p={qaoa_p_param} layers)...")
    initial_gamma = np.random.rand(qaoa_p_param) * (np.pi / 4) 
    initial_beta = np.random.rand(qaoa_p_param) * 2 * np.pi
    initial_params = np.concatenate((initial_gamma, initial_beta))
    bounds = [(0, np.pi / 4)] * qaoa_p_param + [(0, 2 * np.pi)] * qaoa_p_param 
    optimization_result = minimize(
        lambda params: objective_func_wrapper(params, sampler, backend), # Pass sampler and backend to the wrapper
        initial_params,
        method='COBYLA',
        bounds=bounds,
        options={'maxiter': qaoa_p_param * 2 + 3}
    )
    optimal_params = optimization_result.x
    optimal_gamma = optimal_params[:qaoa_p_param]
    optimal_beta = optimal_params[qaoa_p_param:]
    ...
    if method_param == 'node_visit_time':
        optimal_qc = qaoa_circuit_func(num_qubits, qaoa_p_param, optimal_gamma, optimal_beta)
    else: # naive_adjacency
        optimal_qc = qaoa_circuit_func(len(all_nodes), qaoa_p_param, optimal_gamma, optimal_beta)
    optimal_qc_transpiled = transpile(optimal_qc, backend=backend)
    job = sampler.run([optimal_qc_transpiled], shots=shots_param)
    ...
    print(f"All measurement counts from {shots_param} shots:\n{final_counts}")

    data_to_write_for_current_run = []
    min_classical_cost, avg_classical_cost, max_classical_cost = math.inf, 0.0, math.inf
    ...
    if method_param == 'node_visit_time':
        ...
        for bitstring in valid_bitstrings:
            ...
            cost = calculate_route_cost_nvt(route, distance_matrix)
            data_to_write_for_current_run.append([method_param, cluster_type_param, target_id_param, qaoa_p_param, shots_param, bitstring, count, cost, min_classical_cost, avg_classical_cost, max_classical_cost, current_timestamp])
    ...
    with open(CSV_FILE_PATH, 'a', newline='') as csvfile:
        ...
        writer.writerows(data_to_write_for_current_run)

    print(f"\nResults for this run have been appended to '{CSV_FILE_PATH}'")
    print("Experiment finished successfully!")
    return True # Indicate success

# --- Utility to redirect stdout ---
class Tee:
...
def main():
    print("Starting unified QAOA for VRP script with batch processing and checkpointing...")

    # Ensure data_entries.csv exists with header if not present
    if not os.path.exists(DATA_ENTRY_FILE_PATH):
        with open(DATA_ENTRY_FILE_PATH, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['conf_qubit_alloc', 'conf_cluster_type', 'conf_target_id', 'conf_qaoa_depth', 'shots', 'done'])
        print(f"Created empty '{DATA_ENTRY_FILE_PATH}'. Please populate it with experiment configurations.")
        return

    # Read all data entries
    data_entries = []
    try:
        with open(DATA_ENTRY_FILE_PATH, 'r', newline='') as f:
            reader = csv.DictReader(f)
            data_entries = list(reader)
    except FileNotFoundError:
        print(f"Error: '{DATA_ENTRY_FILE_PATH}' not found. Please create it.")
        return
    except Exception as e:
        print(f"Error reading '{DATA_ENTRY_FILE_PATH}': {e}")
        return

    if not data_entries:
        print(f"'{DATA_ENTRY_FILE_PATH}' is empty. No experiments to run.")
        return
    ...
    for i, entry in enumerate(data_entries):
        if entry.get('done', 'no').lower() == 'no':
            ...
            try:
                ...
                print(f"Log started for experiment: {method}, {cluster_type}, {target_id}, P={qaoa_p}, Shots={shots}")

                # Run the experiment
                success = run_single_qaoa_experiment(method, cluster_type, target_id, qaoa_p, shots)

                if success:
                    # Update 'done' status in memory
                    data_entries[i]['done'] = 'yes'
                    print(f"Experiment for entry {i+1} marked as 'done'.")

                    # Save checkpoint CSV
                    current_datetime_str = datetime.now().strftime("%Y%m%d_%H%M%S")
                    checkpoint_csv_name = f"out_{current_datetime_str}.csv"
                    
                    # Read the main CSV and write it to the checkpoint file
                    if os.path.exists(CSV_FILE_PATH):
                        with open(CSV_FILE_PATH, 'r', newline='') as infile:
                            reader = csv.reader(infile)
                            main_csv_content = list(reader)
                        with open(checkpoint_csv_name, 'w', newline='') as outfile:
                            writer = csv.writer(outfile)
                            writer.writerows(main_csv_content)
                        print(f"Checkpoint saved to '{checkpoint_csv_name}'")
                    else:
                        print(f"Warning: '{CSV_FILE_PATH}' not found, skipping checkpoint for this run.")

                else:
                    print(f"Experiment for entry {i+1} failed. 'done' status remains 'no'.")

            except Exception as e:
                print(f"An unexpected error occurred during experiment for entry {i+1}: {e}")
                ...
                print(f"Output for this run also saved to '{log_filename}'") # This prints to original stdout after redirection is off

            # Write updated data_entries back to DATA_ENTRY_FILE_PATH
            try:
                with open(DATA_ENTRY_FILE_PATH, 'w', newline='') as f:
                    fieldnames = data_entries[0].keys() if data_entries else ['conf_qubit_alloc', 'conf_cluster_type', 'conf_target_id', 'conf_qaoa_depth', 'shots', 'done']
                    writer = csv.DictWriter(f, fieldnames=fieldnames)
                    writer.writeheader()
                    writer.writerows(data_entries)
                print(f"'{DATA_ENTRY_FILE_PATH}' updated.")
            except Exception as e:
                print(f"CRITICAL ERROR: Could not write back to '{DATA_ENTRY_FILE_PATH}': {e}")
                print("Manual intervention may be required to prevent data loss.")
        else:
            print(f"Skipping entry {i+1}: Already marked as 'done'.")

    if not processed_any_entry:
        print("\nNo pending experiments found in 'data_entries.csv'. All 'done'.")
    else:
        print("\nAll pending experiments processed (or skipped due to errors). Script finished.")
