...
OUTPUT_GRAPHS_JSON = 'cluster_complete_graphs.json'
...
def build_road_graph(road_df):
    """
    Builds a NetworkX graph from road segments.
    Nodes are unique (lat, lon) points, edges are road segments with Haversine distance as weight.
    """
    ...
    for _, row in road_df.iterrows():
        ...
        # Add edge with Haversine distance as weight
        distance = haversine(start_coord[0], start_coord[1], end_coord[0], end_coord[1])
    ...
    print(f"Road network graph built with {G.number_of_nodes()} nodes and {G.number_of_edges()} edges.")
...
def map_points_to_nearest_road_node(points_df, node_coords):
    ...
    for _, row in points_df.iterrows():
        ...
        # k=1 for single nearest neighbor, distance_upper_bound to limit search radius if desired
        ...
        if index < len(road_node_ids): # Ensure a valid index was returned
            ...
            point_to_node_mapping[row['ID']] = closest_road_node_id
        else:
...
def calculate_complete_graph_distances(cluster_points_data, road_graph, point_to_node_mapping):
    ...
    for original_id in cluster_points_data:
        if original_id in point_to_node_mapping:
            cluster_node_ids.append(point_to_node_mapping[original_id])
            cluster_original_ids.append(original_id)
        else:
            print(f"  Skipping original ID {original_id} as it was not mapped to a road node.")

    if len(cluster_node_ids) < 2:
        print("  Cluster has fewer than 2 mappable points. Skipping distance matrix calculation.")
        return {}

    # Calculate all-pairs shortest paths within this subset of nodes
    # Using Dijkstra's for all-pairs on a subset of nodes, which is more efficient than full all_pairs_shortest_path
    
    # Create a subgraph if the number of nodes is large, or just iterate
    # For a complete graph, we need distances between all pairs.
    # networkx.shortest_path_length can compute from one source to all reachable nodes.
    # We need to do this for every node in the cluster.

    distance_matrix = {}
    for i, source_original_id in enumerate(cluster_original_ids):
        source_node_id = cluster_node_ids[i]
        distance_matrix[source_original_id] = {}
        
        # Check if source_node_id is actually in the graph
        ...
        try:
            # Calculate shortest paths from this source to all other nodes in the cluster
            ...
            # This exception is caught by checking target_node_id in paths_from_source
            pass
        except Exception as e:
            print(f"  Error calculating paths from {source_original_id} (node {source_node_id}): {e}")
            for target_original_id in cluster_original_ids:
                distance_matrix[source_original_id][target_original_id] = None # Mark as error/unreachable
...
def main():
    ...
    if any(df is None for df in [bins_df, waste_areas_df, facilities_df, road_df]) or \
       any(json_data is None for json_data in [litter_bins_clustering, waste_areas_clustering]):
        print("Aborting graph creation due to missing or unreadable input files.")
        return
    ...
    if litter_bins_clustering and 'assignments' in litter_bins_clustering:
        for cluster_assignment in litter_bins_clustering['assignments']:
            waste_area_id = cluster_assignment['waste_collection_area_id']
            public_litter_bin_ids = cluster_assignment['public_litter_bin_ids']

            # Get coordinates for the waste area (centroid)
            waste_area_coords_row = waste_areas_df[waste_areas_df['ID'] == waste_area_id].iloc[0]
            
            # Combine waste area ID and bin IDs for this cluster
            cluster_points_ids = [waste_area_id] + public_litter_bin_ids
            
            print(f"  Processing cluster for Waste Collection Area ID: {int(waste_area_id)} with {len(public_litter_bin_ids)} bins.")
            ...
            all_cluster_graphs["litter_bins_to_waste_areas_graphs"].append({
                "waste_collection_area_id": waste_area_id,
    ...
    if waste_areas_clustering and 'assignments' in waste_areas_clustering:
        for cluster_assignment in waste_areas_clustering['assignments']:
            waste_facility_id = cluster_assignment['waste_facility_id']
            waste_collection_area_ids = cluster_assignment['waste_collection_area_ids']

            # Combine facility ID and waste area IDs for this cluster
            cluster_points_ids = [waste_facility_id] + waste_collection_area_ids
            
            print(f"  Processing cluster for Waste Facility ID: {int(waste_facility_id)} with {len(waste_collection_area_ids)} waste areas.")
            ...
            all_cluster_graphs["waste_areas_to_facilities_graphs"].append({
                "waste_facility_id": waste_facility_id,
                "distance_matrix_km": distance_matrix
            })
    else:
